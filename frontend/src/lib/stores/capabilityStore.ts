...
export const selectedResource = derived([resources, selectedResourceId], ([$resources, $id]) => {
...
export const filteredResources = derived([resources, selectedType], ([$resources, $type]) => {
  if (!$resources) return [];
  if (!$type) {
  ...
  // Match by resource type field, not by object key
  for (const group of Object.values($resources)) {
    const arr = group as CapabilityResource[];
    if (arr.length > 0 && arr[0].type === $type) {
      return arr;
    }
  }
  return [];
