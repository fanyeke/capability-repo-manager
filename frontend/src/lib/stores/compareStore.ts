...
export async function compareRepoWithPack(repoId: string, packId: string): Promise<void> {
  isLoading.set(true);
  error.set(null);
  try {
...
export async function compareRepos(repoIdA: string, repoIdB: string): Promise<void> {
  isLoading.set(true);
  error.set(null);
  try {
    const { invoke } = await import('@tauri-apps/api/core');
