...
fn detects_skill_with_skill_md_uppercase() {
    ...
    assert!(skill.source_path.as_ref().unwrap().contains("skills/my-skill"));
    assert_eq!(skill.scope, "project");

    let meta: serde_json::Value =
        serde_json::from_str(skill.metadata_json.as_ref().unwrap()).unwrap();
...
fn skips_directories_without_skill_md() {
    let dir = setup_temp_repo(&[("skills/empty-dir/.gitkeep", "")]);
...
fn generates_content_hash() {
    let dir = setup_temp_repo(&[("skills/hash-test/SKILL.md", "content for hashing")]);
