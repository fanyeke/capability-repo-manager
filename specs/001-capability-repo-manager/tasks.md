...
- [ ] T041 [US1] Implement GuidedSetup page (first-launch: prompts to add scan directories, "Skip" option) in frontend/src/lib/pages/GuidedSetup.svelte
- [ ] T042 [US1] Implement Dashboard page (main view: RepoList + summary cards for total repos, repos with config, recent scans) in frontend/src/lib/pages/Dashboard.svelte
- [ ] T043 [US1] Implement Settings page (scan roots management, scan depth config, ignore patterns editor, pack storage path) in frontend/src/lib/pages/Settings.svelte
- [ ] T044 [US1] Wire up Tauri invoke calls in repoStore and verify end-to-end scan → display flow works from GuidedSetup → Dashboard
...
- [ ] T059 [P] [US2] Implement ResourceDetail component (name, type, scope badge, file path, git-tracked indicator, dependencies, error display) in frontend/src/lib/components/ResourceDetail.svelte
- [ ] T060 [US2] Implement RepoDetail page (tabs: Overview, Capabilities; top summary bar with repo metadata) in frontend/src/lib/pages/RepoDetail.svelte
- [ ] T061 [US2] Register tauri-bridge commands in main Tauri app and wire frontend RepoDetail → capabilityStore → backend parsers
...
- [ ] T075 [P] [US3] Implement PackLibrary component (pack card grid, name/version/type/source badges, delete action) in frontend/src/lib/components/PackLibrary.svelte
- [ ] T076 [US3] Wire export flow: RepoDetail → PackExport page → pack-engine → PackLibrary display
...
- [ ] T091 [P] [US4] Implement ConflictResolver component (per-item conflict display, strategy selector: skip/overwrite/rename/merge, inline preview) in frontend/src/lib/components/ConflictResolver.svelte
- [ ] T092 [US4] Implement PackApply page (pack selector, target repo selector, dry-run trigger → MigrationPlan → ConflictResolver → execute → report view) in frontend/src/lib/pages/PackApply.svelte
- [ ] T093 [US4] Wire migration flow: PackLibrary → PackApply page → migration-engine → report display
...
- [ ] T109 [US5] Implement Doctor page (run button, report display, historical reports list for repo) in frontend/src/lib/pages/Doctor.svelte
- [ ] T110 [US5] Wire auto-doctor: after migration execution completes → automatically trigger run_doctor → display mini report in migration result
...
- [ ] T117 [US6] Implement Compare page (source selector: repo or pack, target selector: repo, run comparison → CompareView) in frontend/src/lib/pages/Compare.svelte
...
- [ ] T118 [P] Write E2E test for MVP user journey (guided setup → scan → open repo → view capabilities → export pack) in tests/e2e/scan-and-browse.spec.ts
- [ ] T119 [P] Write E2E test for migration user journey (apply pack → review plan → resolve conflicts → execute → view report → run doctor) in tests/e2e/migrate-pack.spec.ts
- [ ] T120 [P] Write E2E test for doctor user journey (open repo → run doctor → view issues → verify score) in tests/e2e/diagnose.spec.ts
- [ ] T121 Performance validation: verify 500-repo scan completes within 30 seconds (SC-001)
- [ ] T122 Performance validation: verify repo detail with 100+ resources renders within 10 seconds (SC-002)
- [ ] T123 [P] Implement background startup refresh of Git metadata for all indexed repos (FR-005 incremental refresh)
- [ ] T124 [P] Implement disk space check before pack creation and migration execution
- [ ] T125 Run quickstart.md validation: verify all setup instructions work on clean Linux environment
- [ ] T126 Final Constitution check: verify all 6 principles pass, code coverage ≥80% across all crates
